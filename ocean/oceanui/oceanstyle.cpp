...
#include "ocean.h"
#include "oceananimations.h"
#include "oceanframeshadow.h"
#include "oceanmdiwindowshadow.h"
#include "oceanmnemonics.h"
#include "oceanpropertynames.h"
#include "oceanshadowhelper.h"
#include "oceansplitterproxy.h"
#include "oceanstyleconfigdata.h"
#include "oceanwidgetexplorer.h"
#include "oceanwindowmanager.h"
#include "oceanblurhelper.h"
...
namespace OceanPrivate
{
    ...
    class TabBarData: public QObject
    {
        ...
        Ocean::WeakPointer<const QWidget> _tabBar;
    ...
    class ComboBoxItemDelegate: public QItemDelegate
    {
        ...
        explicit ComboBoxItemDelegate( QAbstractItemView* parent ):
            ...
            _itemMargin( Ocean::Metrics::ItemView_ItemMarginWidth )
        {}
        ...
        //* proxy
        Ocean::WeakPointer<QAbstractItemDelegate> _proxy;
...
namespace Ocean
{
    ...
    Style::Style():
        ...
        , _tabBarData( new OceanPrivate::TabBarData( this ) )
        #if OCEAN_HAVE_KSTYLE
        , SH_ArgbDndWindow( newStyleHint( QStringLiteral( "SH_ArgbDndWindow" ) ) )
        , CE_CapacityBar( newControlElement( QStringLiteral( "CE_CapacityBar" ) ) )
        #endif
    {

        // use DBus connection to update on ocean configuration change
        auto dbus = QDBusConnection::sessionBus();
        dbus.connect( QString(),
            QStringLiteral( "/OceanStyle" ),
            QStringLiteral( "org.kde.Ocean.Style" ),
            QStringLiteral( "reparseConfiguration" ), this, SLOT(configurationChanged()) );

        dbus.connect( QString(),
            QStringLiteral( "/OceanDecoration" ),
            QStringLiteral( "org.kde.Ocean.Style" ),
            QStringLiteral( "reparseConfiguration" ), this, SLOT(configurationChanged()) );
    ...
    void Style::polish( QWidget* widget )
    {
        ...
        if( qobject_cast<QScrollBar*>( widget ) )
        {
            ...
            if( !hasParent( widget, "QWebView" ) )
            {
                ...
                { itemView->setItemDelegate( new OceanPrivate::ComboBoxItemDelegate( itemView ) ); }
            }
    ...
    void Style::unpolish( QWidget* widget )
    {
        ...
        if ( OceanPrivate::possibleTranslucentToolBars.contains( widget ) ) 
            OceanPrivate::possibleTranslucentToolBars.remove( widget );
                ...
                && widget->testAttribute( Qt::WA_TranslucentBackground ) )
            {
                switch ( widget->windowFlags() & Qt::WindowType_Mask ) {
                    ...
                    case Qt::Sheet: {
                        ...
                        {
                            if( OceanPrivate::possibleTranslucentToolBars.size() == 1 )
                            {
                                QSet<const QWidget *>::const_iterator i = OceanPrivate::possibleTranslucentToolBars.constBegin();
    ...
    void Style::loadConfiguration()
    {
        ...
        // 使用新的调色板系统
        QPalette newPalette = standardPalette();
    ...
    QRect Style::progressBarGrooveRect( const QStyleOption* option, const QWidget* widget ) const
    {
        ...
        const bool horizontal( OceanPrivate::isProgressBarHorizontal( progressBarOption ) );
    ...
    bool Style::drawMenuBarEmptyAreaControl( const QStyleOption* option, QPainter* painter, const QWidget* widget) const
    {
        ...
        if ( OceanPrivate::possibleTranslucentToolBars.isEmpty() ) shouldDrawShadow = true;
    ...
    bool Style::drawMenuBarItemControl( const QStyleOption* option, QPainter* painter, const QWidget* widget) const
    {
        ...
        if ( widget && _helper->titleBarColor( windowActive ).alphaF()*100.0 < 100 && _translucentWidgets.contains( widget->window() ) && StyleConfigData::widgetDrawShadow() )
        {
            ...
            else if ( OceanPrivate::possibleTranslucentToolBars.size() == 1 )
            {
                QSet<const QWidget *>::const_iterator i = OceanPrivate::possibleTranslucentToolBars.constBegin();
                const QToolBar *tb = qobject_cast<const QToolBar*>( *i );
    ...
    */
    ...
    bool Style::isStylableToolbar(const QWidget* w, bool allowInvisible) const  //should be in helper
    {
        ...
        if (QMainWindow *mw = qobject_cast<QMainWindow*>(p))
        {
            ...
            {   
                if( tb->y() == 0 ) {
                    if ( OceanPrivate::possibleTranslucentToolBars.size() == 0 ) {
                        OceanPrivate::possibleTranslucentToolBars.insert( w );
                        return true;
                    }
                    else if ( OceanPrivate::possibleTranslucentToolBars.contains( w ) && OceanPrivate::possibleTranslucentToolBars.size() == 1 ) 
                        return true;
...
//_____________________________________________________________________
QPalette Style::lightModePalette() const
{
    QPalette palette;
...
//_____________________________________________________________________
QPalette Style::darkModePalette() const
{
    QPalette palette;
